...
def _progress_bar(value, min_val=0, max_val=100, low_color='#22c55e', mid_color='#5c6bc0', high_color='#ef4444'):
...
header h1 {{ font-size:34px; font-weight:800; margin-bottom:8px; letter-spacing:-0.5px; position:relative; }}
header p {{ opacity:0.75; font-size:14px; margin-top:10px; position:relative; }}
.nav {{
    display:flex;
...
.summary-card.up-card {{ border-top:4px solid var(--up); }}
.summary-card.down-card {{ border-top:4px solid var(--down); }}
...
.card.up {{ border-top:5px solid var(--up); }}
.card.down {{ border-top:5px solid var(--down); }}
.card.neutral {{ border-top:5px solid var(--neutral); }}
.card-header {{
    display:flex;
