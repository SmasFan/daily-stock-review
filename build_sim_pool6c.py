...
def run_all(start_date="2026-07-01", end_date="2026-09-04"):
    ...
    return {
        "title": "6股池(工富/长电/紫金/万华/宁波银行/雅戈尔) · 三策略",
...
def run_windows():
    ...
    with open(os.path.join(DATA_DIR, "sim_pool6c_trade.json"), "w", encoding="utf-8") as f:
        json.dump(full, f, ensure_ascii=False)
    with open(os.path.join(DATA_DIR, "sim_pool6c_all.json"), "w", encoding="utf-8") as f:
    ...
    print("完成:", os.path.join(DATA_DIR, "sim_pool6c_trade.json"),
          os.path.join(DATA_DIR, "sim_pool6c_all.json"))
